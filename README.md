...
Project spaces for your AI conversations. Organize ChatGPT and Claude threads into named workspaces, search across exports, save in one keystroke. **Local-first, no account, no telemetry, open source.**
...
SpaceMind organises ChatGPT and Claude conversations into **named spaces** — one per project, research thread, or personal context. It sits between AIPRM (closed-source, paid, prompt-library focus) and ChatGPT's built-in folders (single-platform, walled garden), with cross-platform reach, full local ownership, and zero cost.

### Core features

- **Spaces** — group conversations by project. Pin favourites, add an emoji + note, reorder freely.
- **Cross-platform** — one inbox for both ChatGPT and Claude. The same space can hold threads from either.
- **ZIP import** — drop your official ChatGPT or Claude export and search every message you've ever sent, locally.
- **Real-time sidebar scraping** — visit chatgpt.com / claude.ai and SpaceMind silently captures new conversations as you create them. No polling, no API.
- **⌘+Shift+K overlay** — one keystroke on a supported site opens a floating picker to file the current thread into a space.
- **Floating button** — a purple FAB on the right edge of chatgpt.com / claude.ai for users who prefer a visible entry point.
...
- **Minimal permissions**: `storage`, plus host access for `chatgpt.com` and `claude.ai` only (required for sidebar scraping + overlay).
...
5. On chatgpt.com / claude.ai: click the purple floating button (or press `⌘+Shift+K`) to file the current thread into a space.
...
| `⌘+Shift+K` | Toggle the overlay on chatgpt.com / claude.ai |
...
| Tests | Vitest + `fake-indexeddb` (181 tests) |
...
- **v1.2** — Add Gemini, Perplexity, DeepSeek as supported platforms.
...
把 ChatGPT 和 Claude 的对话整理成命名的「项目空间」,本地搜索全部历史,一个快捷键把当前对话归档。**完全本地、无账号、零遥测、开源免费。**

[**安装 →**](https://chromewebstore.google.com/detail/EXTENSION_ID_PLACEHOLDER) · [隐私政策](PRIVACY.md)

### 它是什么

SpaceMind 处在 AIPRM(闭源 / 收费 / 偏 Prompt 库)和 ChatGPT 自带文件夹(单平台 / 封闭生态)之间 — 跨平台、完全本地、开源免费。

### 核心

- **空间(Spaces)** — 按项目分组对话,可置顶、加 emoji + 备注、自由排序。
- **跨平台** — 一个收件箱同时管理 ChatGPT 和 Claude,同一个空间可以混放两边的对话。
- **ZIP 导入** — 把官方导出的 ZIP 丢进来,本地全文搜索每一条历史消息。
- **侧边栏实时抓取** — 在 chatgpt.com / claude.ai 正常使用时,SpaceMind 静默捕获新对话,不轮询、不走 API。
...
- 最小权限:`storage` + 仅针对 chatgpt.com / claude.ai 的站点权限(供侧边栏抓取和浮层使用)。
- MIT 开源,可审计。
